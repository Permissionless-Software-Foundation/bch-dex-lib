/*
  This class library governs the action of Takers or *take* an Offer by
  generating a Counter Offer and uploading it to Nostr relays.

  TODO:
  - Initialize Nostr adapter with a pool of relays.
    - User should be able to define the array of relays.
      - Use these relays by default:
        - 'wss://nostr-relay.psfoundation.info',
        - 'wss://nos.lol',
        - 'wss://relay.damus.io'
  - Remove uploadCounerOffer() and replace with uploadCounterOffer2()
...
class Take {
  constructor (localConfig = {}) {
    ...
    // Instantiate nostr with default pool relays.
    this.nostr = new NostrAdapter({ nostrTopic: 'bch-dex-test-topic-01' })
  ...
  async takeOffer (cid) {
    try {
      ...
      const offerData = await this.retryQueue.addToQueue(this.util.getEntryFromNostr, cid)
      console.log('offerData: ', offerData)

      // Verify the UTXO is still valid.
      const utxo = {
        tx_hash: offerData.utxoTxid,
        tx_pos: offerData.utxoVout
      }
      ...
      return { offerData, partialHex }

      // Initialize the Nostr adapter
      // const wif = utxoInfo.wif
      // await this.nostr.start(wif)

      // // Upload the Counter Offer to a Nostr relay
      // const offerCid = cid
      // const partialHex = hex
      // const {eventId, noteId} = await this.retryQueue.addToQueue(this.uploadCounterOffer2, { offerData, partialHex })
  ...
  async uploadCounterOffer (inObj = {}) {
    try {
      ...
      const resultEventIds = await this.nostr.post(JSON.stringify(nostrData))
      // const resultEventId = 'fake-event-ID'

      const noteIds = []
      for (let i = 0; i < resultEventIds.length; i++) {
        const noteId = this.nostr.eventId2note(resultEventIds[i])
        noteIds.push(noteId)
      }

      return { eventIds: resultEventIds, noteIds }
    } catch (err) {
  ...
  async generatePartialTx (inObj = {}) {
    try {
      ...
      // Calculate how much to send to the bch-dex operator for their fee.
      let operatorSatsToReceive = Math.ceil(satsNeeded * offerInfo.operatorPercentage / 100)
      if (operatorSatsToReceive < 546) operatorSatsToReceive = 546
      console.log(`operatorSatsToReceive: ${operatorSatsToReceive}`)

      // Send the operator fee to the bch-dex operator.
      transactionBuilder.addOutput(offerInfo.operatorAddress, operatorSatsToReceive)
  ...
  async moveBch (offerData = {}, hdIndex = 1) {
    ...
    let satsToMove = Math.ceil(offerData.numTokens * parseInt(offerData.rateInBaseUnit))

    if (isNaN(satsToMove)) {
      throw new Error('Could not calculate the amount of BCH to generate counter offer')
    }

    // Add the operator fee to the sats needed.
    let operatorFee = Math.ceil(satsToMove * offerData.operatorPercentage / 100)
    if (operatorFee < 546) operatorFee = 546
    satsToMove += operatorFee
  ...
  async ensureFunds (offerData) {
    // console.log('ensureFunds() offerData: ', offerData)
    ...
    const hasEnoughSats = 600
    const bchAddr = this.bchWallet.walletInfo.cashAddress

    if (offerData.buyOrSell.includes('sell')) {
      // Sell Offer

      // Calculate the sats needed to purchase the tokens.
      const satsNeeded = Math.ceil(offerData.numTokens * parseInt(offerData.rateInBaseUnit))
      if (isNaN(satsNeeded)) {
        throw new Error('Could not calculate sats needed!')
      }

      // Ensure the app wallet controls enough BCH to pay for the tokens.
